...
#### `aksGenerateAadPodIdBindings` (boolean)

AKS specific. Using Managed Identities requires the generation labels for [aadpodidbing](https://docs.microsoft.com/en-us/azure/aks/use-azure-ad-pod-identity). 
When set to true these labels will be generated by convention, unless overriden at the project or ingress level.
The convention is: 
    $"{application.Name}-{project.Name}-service"; // for services
    $"{application.Name}-{ingress.Name}-ingress"; // for ingresses
...
#### `aksPodIdentityName` (string)

An optional name of pod identity to use for the service when generating the aadpodidbinding label.
...
An optional name of pod identity to use for the ingress when generating the aadpodidbinding label.
